function Extended_Data_Figure_2_Panel_k

filename = "Networks_N60.txt";

L = dlmread(filename);

T_index = 2;    %  T_index = 1 means inter-network transition probabilities q_{12} = q_{21} = 10^{-4}   
...
d_Trust = zeros(2, SZ); %%  derivative of trust, \bar{p}
...
for iter = 1:SZ 
    ...
    B = 1.5;
    d_Trust(1, iter) = (B-1)/12*Lambda_static(1) - Lambda_static(2)/12 + (B-2)/24* Lambda_static(3);
    d_Trust(2, iter) = (B-1)/12*Lambda_dynamic(1) - Lambda_dynamic(2)/12 + (B-2)/24* Lambda_dynamic(3);
end
...
for k = 1:10
    scatter( ...
        Trust(1, idx{k}), ...   % static
        Trust(2, idx{k}), ...   % dynamic
        marker_size, ...
        colors(k,:), ...
        'filled', ...
...
axis([0.438 0.502 0.438 0.502]);
yticks(0.44:0.02:0.50);
xtickformat('%.2f');
ytickformat('%.2f');
